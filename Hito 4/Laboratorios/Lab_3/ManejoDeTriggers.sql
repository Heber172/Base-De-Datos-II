...
##################3##########################################################################################################################################################################
#AUDITORIa: ES EL HECHO DE PODER MONITOREAR TODAS LAS ACCIOENS QUE OCURRE EN UNA TABLA
...
insert into usuarios_rrhh(id_usr, nombre_completo, fecha_nac, correo, password) values(123456, 'Heber Miranda', '2003-11-02', 'heber@gmail.com', '12345674');

select * from usuarios_rrhh;

##################3##########################################################################################################################################################################
#me permite obtener fecha actual
select current_date();

#me permite obtener fecha actual y hora
select now();
...
#este comando me permite ver todas las variables de la base de dats
show VARIABLES;
##################3##########################################################################################################################################################################
...
insert into usuarios_rrhh(id_usr, nombre_completo, fecha_nac, correo, password) values(001234, 'Danner', '1999-11-02', 'danner@gmail.com', '45689');
...
#ACTUALIZAR DATOS
create or  replace trigger tr_audit_usuarios_rrhh_v3
    after update
    on usuarios_rrhh
    for each row
    begin
        insert into audit_usuarios_rrhh(fecha_mod, usuario_log, hostname, accion, antes_del_cambio, despues_del_cambio)
        select now(),user(), @@hostname, 'UPDATE', CONCAT(old.id_usr,' ', old.nombre_completo,' ', old.fecha_nac), concat(new.id_usr,' ', new.nombre_completo,' ', new.fecha_nac);
    end;


insert into usuarios_rrhh(id_usr, nombre_completo, fecha_nac, correo, password) values(1111111, 'Sergio', '1999-11-02', 'mateo@gmail.com', '45689');
...
begin
    insert into audit_usuarios_rrhh(fecha_mod, usuario_log, hostname, accion, antes_del_cambio, despues_del_cambio) values(fecha, usuario, hostname, accion, antes, despues);
end;
